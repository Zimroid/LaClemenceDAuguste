...
import octavio.server.command.server.RoomClose;
...
public class Server extends WebSocketServer
{
    ...
    public void onClose(WebSocket socket, int code, String reason, boolean byRequest)
    {
        ...
        if (this.clients.get(socket) != null)
        {
            this.logOut(this.clients.get(socket));
        }
    ...
    public void onMessage(WebSocket socket, String message)
    {
        ...
        try
        {
            ...
            {
                command.setUser(this.clients.get(socket));
            }
            if (command.checkRoom())
            {
                command.setRoom(this.rooms.get(json.getInt("room_id")));
            }
     ...
     *
     * @param socket WebSocket concernée, null en cas d'erreur interne au
     *               serveur
    ...
    public void onError(WebSocket socket, Exception e)
    {
        ...
        if (socket != null)
        {
            Log.error(socket.getRemoteSocketAddress() + ": Error");
        }
        else
        {
    ...
    public void broadcast(String message)
    {
        ...
        {
            this.send(user.getSocket(), message);
        }
    }
    ...
    public void logOut(User user)
    {
        ...
        for (Room room : ((HashMap<Integer, Room>)user.getRooms().clone()).values())
        {
        ...
        {
            ClientCommand.sendError(user.getSocket(), "logged_out");
        }
    ...
    public Room getRoom(int id) throws InexistantRoomException
    {
        ...
        {
            throw new InexistantRoomException(id);
        }
    }
    ...
    public Room createRoom(String name)
    {
        ...
        synchronized (this.rooms)
        {
            ...
            roomId = 1;
            while (this.rooms.containsKey(roomId))
            {
                roomId++;
            }
    ...
    public void closeRoom(Room room)
    {
        ...
        for (User user : ((HashMap<Integer, User>)room.getUsers().clone()).values())
        {
            this.leaveRoom(user, room);
        }
    }
    ...
    {
        for (User user : this.getUsersWatchers())
        {
            this.send(user.getSocket(), (new QueryUsers()).toString());
        }
    ...
    {
        for (User user : this.getRoomsWatchers())
        {
            this.send(user.getSocket(), (new ListGames()).toString());
        }
