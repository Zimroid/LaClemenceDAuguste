 ...
 * Permet d'assurer le sotckage des utilisateurs dans la BDD
 * @author Vr4el
 */
public class UserManager
{
    ...
    {
        String query = String.format("SELECT id, username, password FROM user WHERE login = '%s'", login);
        ResultSet rs = connect.executeQuery(query);
    ...
    {
        String query = String.format("SELECT id, username, password FROM user WHERE login = '%s'", nom);
        ResultSet rs = connect.executeQuery(query);
        return rs.next();
    }
}
